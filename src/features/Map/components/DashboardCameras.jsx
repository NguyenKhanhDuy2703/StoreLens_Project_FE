...
const DashboardCameras = ({ cameras = []  , updateDataCamera}) => {
  const totalZones = cameras.reduce((sum, cam) => sum + cam.zones.length, 0);
  const camerasWithImage = cameras.filter(c => c.image).length;
  const camerasNeedSetup = cameras.filter(c => !c.image || c.zones.length === 0).length;
  return (
            ...
            {cameras.map(cam => (
              ...
              >
                ...
                <div className="flex items-center space-x-4 mb-4 text-sm">
                  <div className={`flex items-center ${cam.image ? 'text-green-600' : 'text-gray-400'}`}>
