...
const DashboardCameras = ({ cameras = [], metricCameraZones }) => {
  const { totalZones, cameraHaveImage: camerasWithImage, cameraNotSettuped: camerasNeedSetup } = metricCameraZones;
  const updateDataCamera = (camera) => {
    alert(`Cập nhật dữ liệu cho camera: ${camera.cameraName}`);
          ...
          {cameras.length === 0 ? (
            ...
            <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
              {cameras.map((cam) => (
                <div
                  key={cam.cameraCode}
                  className="border-2 border-gray-200 rounded-xl p-6 hover:border-purple-400 hover:shadow-lg transition-all bg-white"
                >
                  {/* Camera Header */}
                      ...
                      <div className="flex items-center justify-between text-sm">
                        <div className="flex items-center text-indigo-700">
                          ...
                          <span className="ml-2 font-semibold">
                            {cam.zones.width_frame} x {cam.zones.height_frame}
                          </span>
                        </div>
                        {cam.zones.background_image && (
                  ...
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    >
                      ...
                      {cam.zones?.background_image ? "Có ảnh nền" : "Chưa có ảnh"}
                    </div>
                    <div
                      className={`flex items-center text-sm px-3 py-1.5 rounded-full ${
                        cam.zones?.zones?.length > 0
                    ...
                    {cam.zones?.zones?.length > 0 ? (
                      <div className="space-y-2 max-h-48 overflow-y-auto pr-2 custom-scrollbar">
                        {cam.zones.zones.map((zone, idx) => (
                          <div
                            key={zone.zone_id}
                            className="flex items-center space-x-3 text-sm bg-gradient-to-r from-gray-50 to-white rounded-lg p-3 border border-gray-200 hover:border-purple-300 hover:shadow-sm transition-all"
                          >
